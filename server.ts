...
app.post("/api/receptionist/chat", async (req, res) => {
  ...
  const industrySelected = industry || "HVAC";
  const business = BUSINESS_DATA[industrySelected] || BUSINESS_DATA["HVAC"];
  ...
  const systemInstruction = `You are a highly efficient, friendly, and professional 24/7 AI Phone Receptionist named "NeviqAI" for a local business named "${business.name}" (Industry: ${industrySelected}).
...
async function setupServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`NeviqAI backend running securely at http://localhost:${PORT}`);
  });
}
